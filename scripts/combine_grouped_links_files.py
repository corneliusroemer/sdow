...
if not OUTGOING_LINKS_FILE.endswith('.zst'):
  print('[ERROR] Outgoing links file must be zst compressed.')
  sys.exit()

if not INCOMING_LINKS_FILE.endswith('.zst'):
  print('[ERROR] Incoming links file must be zst compressed.')
  sys.exit()

# Create a dictionary of page IDs to their incoming and outgoing links.
LINKS = defaultdict(lambda: defaultdict(str))
for line in zstandard.open(OUTGOING_LINKS_FILE, 'r'):
  [source_page_id, target_page_ids] = line.rstrip('\n').split('\t')
  LINKS[source_page_id]['outgoing'] = target_page_ids

for line in zstandard.open(INCOMING_LINKS_FILE, 'r'):
